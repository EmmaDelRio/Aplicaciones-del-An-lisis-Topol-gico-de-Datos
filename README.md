...
En este repositorio se encuentran los códigos que corresponden a la implementación en Python de lo presentado en la memoria del TFG Aplicaciones del Análisis Topológico de Datos (TDA). El trabajo se centra en explicar una aplicación
para cada una de las técnicas destacadas del TDA: algoritmo Mapper y homología persistente. El capítulo 2  corresponde a la aplicación del algoritmo Mapper al conjunto de electrocardiogramas Icentia 11k con el objetivo de estudiar si el algoritmo es
capaz de revelar patrones subyacentes de las señales que indiquen un nuevo subtipo de arritmia. El capítulo 3, al estudio de la utilidad de la homología persistente combinada con redes neuronales convolucionales para la clasificación comportamiento caótico - no caótico 
aplicado al sistema de Lorenz ya  la ecuación logística, con el objetivo de estudiar si la información aportada por la homología persistente puede sustituir otras técnicas como el cálculo del exponente de Lyapunov.
...
desarrolla al final del capítulo 2. Para ejecutar los códigos de las configuraciones es necesario que el documento X_mapper.npy (en mi caso es el conjunto de entrada resultado del preprocesado, pero si se quisieran aplicar a otros conjuntos debería ser el conjunto
al que se desea aplicar el algoritmo, una matriz (n,15)) en la misma carpeta que los códigos.
...
Deep Learning for chaos detection (de R. Barrio, Á. Lozano, A. Mayora-Cebollero, A. Miguel, A. Ortega, S. Serrano y R. Vigara)). Para ejecutar los códigos del sistema de Lorenz y la parte de comparación de la ecuación logística es necesario modificar las rutas a los
archivos que contienen los conjuntos de datos de entrenamiento, test y validación.
